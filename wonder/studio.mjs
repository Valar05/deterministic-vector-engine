import {EVIDENCE_AXES,MAX_VISIBLE_STROKES,REVIEW_SCALES,VIEWS,acceptanceFailures,allStrokes,markSourceVerified,promoteUserAccepted,recordScaleReview,renderMechanismSvg,setAblationEvidence,validateMechanismStudy,visibleStrokes} from '../src/sparse-line-study.mjs';
...
const starter=curriculum.study,storageKey='empty-glass-mechanism-v4:'+curriculum.sourceImage.sha256;
let study=restore(),view='ASSEMBLED',referenceVisible=true,mapVisible=true,activeScale=96,ablationIndex=-1,sourceHref=null,sourceObjectUrl=null,verifiedSourceSha=null;
...
function drawMain(){
 ...
 const rendered=parseSvg(renderMechanismSvg(study,{view,omitStrokeId:omittedId(),showMap:mapVisible}));
...
function drawPreviews(){
 ...
 for(const size of REVIEW_SCALES){const card=document.createElement('button');card.className='preview-card'+(activeScale===size?' active':'');card.style.setProperty('--review-size',size+'px');card.dataset.scale=size;const category=study.review[view].CATEGORY.find(item=>item.size===size).verdict||'CATEGORY ?';const structure=study.review[view].STRUCTURE.find(item=>item.size===size).verdict||'STRUCTURE ?';card.innerHTML='<span>'+size+' px · '+category+' · '+structure+'</span>';card.append(parseSvg(renderMechanismSvg(study,{view,omitStrokeId:omittedId()})));card.addEventListener('click',()=>{activeScale=size;render()});host.append(card)}
}
function drawLedger(){
 ...
 const graph=document.createElement('ol');for(const edge of study.attachments){const item=document.createElement('li');item.textContent=edge.id+' · '+edge.partIds.join(' ↔ ')+' · '+edge.state;graph.append(item)}host.append(graph);
}
function blockers(){const result=acceptanceFailures(study);if(referenceVisible)result.push('CURRENT_REFERENCE_VISIBLE');if(ablationIndex>=0)result.push('ABLATION_ACTIVE');return[...new Set(result)]}
function render(){
 ...
 for(const button of document.querySelectorAll('[data-axis]')){button.disabled=!current||referenceVisible||!sourceVerified();button.classList.toggle('active',Boolean(current&&current.evidence[view][button.dataset.axis]===button.dataset.value))}
 for(const button of document.querySelectorAll('[data-review-axis]')){const axis=button.dataset.reviewAxis,item=study.review[view][axis].find(entry=>entry.size===activeScale);button.disabled=referenceVisible||!sourceVerified()||Boolean(current);button.classList.toggle('active',item.verdict===button.dataset.verdict)}
 $('#review-lock').textContent=!sourceVerified()?'Verify the sealed modern source before judgment.':referenceVisible?'Hide the reference before recording judgment.':current?'Return to the full drawing for scale judgment.':'Judge both category and construction at '+activeScale+' px.';
...
function switchView(next){if(next===view)return;view=next;referenceVisible=true;activeScale=96;ablationIndex=-1;render()}
...
$('#ablation-next').addEventListener('click',()=>{const count=currentStrokes().length;ablationIndex=count?(ablationIndex+1)%count:-1;render()});
$('#ablation-prev').addEventListener('click',()=>{const count=currentStrokes().length;ablationIndex=count?(ablationIndex<=0?count-1:ablationIndex-1):-1;render()});
...
$('#reset').addEventListener('click',()=>{if(!confirm('Reset all paired evidence to the source-derived starter?'))return;study=clone(starter);if(verifiedSourceSha)study=markSourceVerified(study,verifiedSourceSha);view='ASSEMBLED';referenceVisible=true;mapVisible=true;activeScale=96;ablationIndex=-1;render()});
